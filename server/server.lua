...
AddEventHandler('rms-wildhorsestable:server:WildHorseStable', function(modelHash, horsename, gender)
    ...
    if Player.Functions.RemoveItem(requiredItem, 1) then
        ...
        -- Insert the wild horse into the 'player_horses' table as a wild horse, with 'valentine' as the default stable
        MySQL.Async.insert('INSERT INTO player_horses(stable, citizenid, horseid, name, horse, gender, active, wild, components, born) VALUES(@stable, @citizenid, @horseid, @name, @horse, @gender, @active, @wild, @components, @born)', {
            ['@stable'] = 'valentine',
            ...
            ['@active'] = false,
            ['@wild'] = true,
            ['@components'] = json.encode({}), -- Empty JSON object for components
            ['@born'] = os.time() -- Current timestamp for born date
        }, function(inserted)
            if inserted then
